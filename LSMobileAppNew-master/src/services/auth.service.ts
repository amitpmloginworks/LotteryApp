...
export class AuthService {
    ...
    getJWTToken(email: string, password: string) {
        ...
        let action = CommonService.version + '/user/session';
        ...
        let opt: RequestOptions = new RequestOptions({
            headers: CommonService.getDFHeaderUser()
        });

        console.log("jwtRequest", jwtRequest);
        var response = this.http.post(CommonService.apiDFUser + action, jwtRequest, opt).map(res => res.json());
        return response;
    }

    refreshJWT(currentJwt: any) {
        ...
        var response = this.http.put('http://api3.hatchster.com/api/v2/user/session', '', opt).map(res => res.json());
        return response;

    }

    forgotPassword(mobile: string, country_code: string) {
        console.log("AuthService::loginUser", CommonService.sessionId, CommonService.isOnline);
        if (!CommonService.sessionId || CommonService.sessionId == "") {
            CommonService.sessionId = this.makeId();
        }
        ...
        var response = this.http.post(action, forgotPassword, opt).map(res => res.json());
   ...
   */
    ...
    uploadProfilePic(filePath: string) {
        ...
        let server = CommonService.apiUrl +
            CommonService.version + '/upload/?process=profile&module_name=image_upload';
