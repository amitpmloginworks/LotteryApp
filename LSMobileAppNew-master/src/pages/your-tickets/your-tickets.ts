...
export class YourTicketsPage {
  ...
  constructor(public appSound: AppSoundProvider, public navCtrl: NavController, public navParams: NavParams, public viewCtrl: ViewController, public _syndService: SyndicateService, public loadingCtrl: LoadingController) {
    ...
    this._syndService.getTickets(parseInt(pid), this.sid, this.stype)
      .subscribe((res) => {
        if (res.response[0].get_tickets.response.status != 'FAIL') {
          this.allTicketData.push(res.response[0].get_tickets.response.ticket_group);
          if (this.allTicketData.length == len) {
            loader.dismiss();
            this.haveData = true
            this.renderData();
          }
          else if (this.allTicketData.length == 0) {
            this.noData = true
            loader.dismiss();
          }
        }
